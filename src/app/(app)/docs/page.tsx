    ...
    <div className="space-y-8">
      ...
      <section className="grid gap-4 lg:grid-cols-3">
        {PDFS.map((p) => (
          <a
            ...
            target="_blank"
            className="group flex flex-col rounded-2xl border border-slate-200 bg-white p-5 shadow-sm transition-colors duration-200 hover:border-accent-300 hover:bg-accent-50/40"
          >
            ...
            <p className="mt-6 text-base font-bold text-slate-900">{p.title}</p>
            <p className="mt-1 text-xs text-slate-500">{p.subtitle}</p>
            <p className="mt-4 font-mono text-[10px] text-slate-400">{p.file}</p>
          </a>
        ))}
      </section>
      ...
      <section>
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {SOPS.map((s) => (
            <Link key={s.id} href={`/docs/sop/${s.id}`} className="group rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-accent-300 hover:shadow-md">
              <div className="flex items-center justify-between">
                <span className="font-mono text-[11px] font-semibold text-accent-600">{s.id}</span>
                <span className="text-[11px] text-slate-400">~{s.estimatedMinutes} min</span>
              </div>
              <p className="mt-2 text-sm font-semibold leading-snug text-slate-900 group-hover:text-accent-700">{s.title}</p>
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        ...
        <Card>
          ...
          <details className="border-t border-slate-100 px-5 py-3">
            <summary className="cursor-pointer text-xs font-semibold text-accent-600">Reference Prisma models</summary>
      ...
      <section className="grid gap-6 xl:grid-cols-3">
        ...
        <Card>
          ...
          <div className="space-y-3 p-5 text-xs text-slate-600">
            ...
            {sample ? (
              <Link href={`/r/${sample.qrToken}`} target="_blank" className="mt-2 inline-flex items-center gap-1 font-semibold text-accent-600 hover:text-accent-500">
